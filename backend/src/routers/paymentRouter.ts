...
router.use('/paddle-webhook', (req: PaddleRequest, res: Response, next: NextFunction) => {
  let data = '';
...
router.get('/payment-status', isAuth, async (req, res) => {
  try {
    ...
    const user = await UserModel.findById(userId).select('isPremium paymentStatus paddleSubscriptionId');
