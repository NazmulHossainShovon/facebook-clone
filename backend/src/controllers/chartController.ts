...
export const checkChartLimit = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (user.remainingChartsLimit && user.remainingChartsLimit > 0) {
    ...
    res.status(200).json({ 
      ...
      message: 'Chart generation limit reached. Please upgrade your account to generate more charts.'
    });
  }
});

export const useChartLimit = asyncHandler(async (req: Request, res: Response) => {
  // Get user ID from the authenticated user (assuming isAuth middleware is used)
  const userId = (req as any).user._id;
