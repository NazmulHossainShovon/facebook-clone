...
import EmployeeDetailsModal from 'components/time-off/EmployeeDetailsModal';
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
              {coverageData.map((day, index) => (
                <div
                  key={index}
                  className={`p-3 rounded-lg text-center bg-neutral-100 cursor-pointer hover:opacity-90`}
                  onClick={() => handleDayClick(day)}
                >
                  <div className="font-medium">{formatDate(day.date)}</div>
                  <div className="text-sm mt-1">
                    <span
                      className={`${day.isGap ? 'text-status-warning' : 'text-status-success'} font-semibold`}
                    >
                      {day.availableCount}
                    </span>{' '}
                    of {day.totalMembers} available
                  </div>
                </div>
              ))}
            </div>
