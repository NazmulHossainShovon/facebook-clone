...
export const createContourChart = (
  ...
  const col = selectedNumericColumn && (numericColumns.includes(selectedNumericColumn) || headers.includes(selectedNumericColumn))
  ...
  const layout = createLayout(`Contour Plot: ${col}`, 'X axis', 'Y axis');
